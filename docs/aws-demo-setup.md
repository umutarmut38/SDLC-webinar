...
Cleanup is manual and should be reviewed in the AWS console before execution:

1. Disable the dedicated CloudFront distribution, wait for deployment, then delete it.
2. Empty and delete only the dedicated demo bucket.
3. Delete only the dedicated deployment role and its inline/attached demo policy.
4. Remove the GitHub `demo` environment variables and any environment approvals.
5. Remove the account OIDC provider only if it was created solely for this repository and no other repository trusts it.

Never use wildcard cleanup scripts. Record resource IDs before the webinar so each deletion can be matched to the dedicated demo environment.
